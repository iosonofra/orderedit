    ...
    <nav class="navbar">
      ...
      <div class="navbar-meta">
        <span v-if="spreadsheetStore.filename" class="filename-badge" :class="{ 'has-unsaved': spreadsheetStore.isUnsaved }">
          <svg class="filename-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
          ...
          <div class="modal-card-body">
            <div class="shortcuts-grid">
              ...
              <div class="shortcut-group">
                ...
                <div class="shortcut-item" v-if="aiStore.enabled">
                  <span class="shortcut-desc">Elabora con AI</span>
                  <span class="shortcut-dots"></span>
                  <span class="shortcut-keys"><kbd>Ctrl</kbd><span class="shortcut-connector">+</span><kbd>Shift</kbd><span class="shortcut-connector">+</span><kbd>A</kbd></span>
                </div>
              </div>
              <div class="shortcut-group">
                <div class="shortcut-group-title">
...
function triggerSmartAi() {
  window.dispatchEvent(new CustomEvent('trigger-smart-ai'))
}
...
function handleGlobalKeyDown(e) {
  ...
  // Ctrl+Shift+A for Smart AI
  if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.key.toLowerCase() === 'a') {
    if (isEditable) return
    e.preventDefault()
    if (aiStore.enabled && spreadsheetStore.hasData && !aiStore.isProcessing) {
      triggerSmartAi()
    }
    return
  }
...
.segmented-control {
  ...
  box-shadow: inset 0 1px 2px rgba(0, 0, 0, 0.4), 0 4px 12px rgba(0, 0, 0, 0.15);
  transition: all 0.3s cubic-bezier(0.16, 1, 0.3, 1);
}

:root[data-theme='light'] .segmented-control {
  background: rgba(235, 240, 250, 0.75);
  border-color: rgba(55, 104, 214, 0.12);
  box-shadow: inset 0 1px 2px rgba(55, 104, 214, 0.05), 0 4px 12px rgba(55, 104, 214, 0.05);
}
...
:root[data-theme='light'] .nav-link:hover {
  background: rgba(55, 104, 214, 0.04);
}
...
.nav-link--active {
  ...
  font-weight: 600;
  box-shadow: var(--shadow-accent-glow), inset 0 1px 0 rgba(255, 255, 255, 0.15);
}
...
:root[data-theme='light'] .nav-link--active {
  ...
  box-shadow: var(--shadow-accent-glow), inset 0 1px 0 rgba(255, 255, 255, 0.25);
}
...
:root[data-theme='light'] .filename-badge {
  background: rgba(0, 0, 0, 0.01);
}

:root[data-theme='light'] .filename-badge.has-unsaved {
...
/* Smart AI - Google Search style */
.ai-search-btn-wrapper {
  position: relative;
  display: inline-flex;
  align-items: center;
  margin-left: 8px; /* staccato dal menu */
  z-index: 10;
}

/* Outer glow wrapper using pseudo-element */
.ai-search-btn-wrapper::before {
  content: '';
  position: absolute;
  top: -2.5px;
  left: -2.5px;
  right: -2.5px;
  bottom: -2.5px;
  background: linear-gradient(135deg, #34a853, #4285f4, #78ffd6, #fbbc05);
...
.ai-search-btn-wrapper:hover::before {
  filter: blur(6px);
  opacity: 0.95;
  top: -3.5px;
  left: -3.5px;
...
.ai-search-btn-wrapper.disabled::before {
  opacity: 0;
  filter: none;
}
...
:root[data-theme='light'] .ai-search-btn {
...
:root[data-theme='light'] .ai-search-btn:hover:not(:disabled) {
  background: #ffffff;
  border-color: rgba(0, 0, 0, 0.12);
  color: #0b57d0; /* Soft google blue on text hover */
}

:root[data-theme='dark'] .ai-search-btn:hover:not(:disabled) {
  background: #252a37;
...
.ai-search-btn:hover:not(:disabled) .ai-sparkle-icon {
  transform: rotate(8deg) scale(1.1);
}
...
:root[data-theme='dark'] .spinner-small {
